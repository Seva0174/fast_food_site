...
CREATE TABLE panier_items (
    id          SERIAL PRIMARY KEY,
    id_panier   INTEGER NOT NULL REFERENCES panier(id) ON DELETE CASCADE,
    id_produit  INTEGER NOT NULL REFERENCES produit_menu(id) ON DELETE CASCADE,
...
CREATE TABLE commandes_menu (
    ...
    id_produit  INTEGER NOT NULL REFERENCES produit_menu(id) ON DELETE RESTRICT,
    quantite    INTEGER NOT NULL CHECK (quantite > 0),
...
CREATE TABLE employe_heure (
    id          SERIAL PRIMARY KEY,
    id_employe  INTEGER NOT NULL REFERENCES employe(id) ON DELETE CASCADE,
...
CREATE TABLE catalogue_fournisseur (
    id              SERIAL PRIMARY KEY,
    id_fournisseur  INTEGER NOT NULL REFERENCES fournisseur(id) ON DELETE CASCADE,
    id_stock        INTEGER NOT NULL REFERENCES stock_matiere_premiere(id) ON DELETE CASCADE,
...
CREATE TABLE recette (
    id                  SERIAL PRIMARY KEY,
    id_produit          INTEGER NOT NULL REFERENCES produit_menu(id) ON DELETE CASCADE,
    id_matiere          INTEGER NOT NULL REFERENCES stock_matiere_premiere(id) ON DELETE RESTRICT,
    quantite_requise    NUMERIC(10,3) NOT NULL
);

CREATE TABLE commandes_fournisseurs_details (
    id                          SERIAL PRIMARY KEY,
    id_commande_fournisseur    INTEGER NOT NULL REFERENCES commandes_fournisseurs(id) ON DELETE CASCADE,
    id_stock                   INTEGER NOT NULL REFERENCES stock_matiere_premiere(id) ON DELETE RESTRICT,
